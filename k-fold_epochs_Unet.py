#
# Script to determine the right number of epochs using K-Fold
#
...
# Paths
DATASET_ROOT = "../../../scratch/izar/suply/dataset/dataset"
...
def k_fold_experiment(lr, ce_weight, dice_weight, full_train_ds, train_ds_for_val, model_name, device='cuda', save_checkpoints=True): 
    ...
    run = wandb.init(
                ...
                project=f"Glacier_Segmentation_{model_name}_epochs",
                group=f"{config_tag}",
                name=f"{config_tag}",
                config={
                    "lr": lr,
...
def main():
    ...
    # Final results            
    results = k_fold_experiment(lr, ce_weight, dice_weight, full_train_ds, train_ds_for_val, model_name, device, save_checkpoints)
